...
final class PlaybackCoordinatorTests: XCTestCase {
    ...
    func testManualTakeoverDuringSuspendedExecuteInvalidatesLateConfirmation() async {
        let adapter = SuspendingExecutePlaybackAdapter(
            snapshots: [.playing(speed: 1.5), .playing(speed: 1.0)]
        )
        let coordinator = PlaybackCoordinator(mode: .speedControl, adapter: adapter)
        ...
        coordinator.applyPushedPlayerSnapshot(.playing(speed: 2.0))
        let stateAfterTakeover = coordinator.state
        ...
        XCTAssertEqual(stateAfterTakeover.stoppedReason, .manualPlayerTakeover)
        XCTAssertEqual(coordinator.state, stateAfterTakeover)
    }
    ...
    func testActivePendingRestoreSpeedConfirmationStopsForNonEchoSpeedTakeover() async {
        let adapter = StreamingStatusPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .playing(speed: 1.0),
    ...
    func testPendingPauseManualPausePressedStopsWithoutLaterResume() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .playerUnavailable,
    ...
    func testPendingPauseManualSpeedChangeConfirmsPauseWithoutStoppingMonitoring() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .playerUnavailable,
        ...
        XCTAssertTrue(coordinator.state.isPlayerControllable)
        XCTAssertEqual(
            coordinator.state.playerSnapshot,
            PlayerSnapshot(playbackState: .paused, speed: 1.25, manualAction: .speedChanged)
        )
        XCTAssertNil(coordinator.state.stoppedReason)
        XCTAssertFalse(adapter.commands.contains(.resume))
    ...
    func testPendingHoldManualSpeedChangeToTargetStopsWithoutLaterRestore() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .playerUnavailable,
    ...
    func testPendingResumeManualPausePressedStopsAndIgnoresLateResumeEcho() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .paused(speed: 1.5),
    ...
    func testPendingResumeManualSpeedChangeRetainsPendingResumeWithoutStoppingMonitoring() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .paused(speed: 1.5),
        ...
        XCTAssertEqual(completedActions, [.pausedByGlanceHold, .resumedPlayback])
        XCTAssertTrue(coordinator.state.isPlayerControllable)
        XCTAssertEqual(coordinator.state.playerSnapshot, .playing(speed: 1.25))
        XCTAssertNil(coordinator.state.stoppedReason)
    }

    func testSupersededPendingPauseLandingWhileFacingRequestsManualTakeoverStop() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .playerUnavailable,
    ...
    func testSupersededPendingRestoreKeepsOriginalSpeedUntilTrustedConfirmation() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .playing(speed: 1.0),
        ...
        XCTAssertNil(coordinator.state.stoppedReason)
        XCTAssertEqual(coordinator.state.playerSnapshot, .playing(speed: 1.5))
        XCTAssertFalse(adapter.commands.contains(.restoreSpeed(1.0)))
        assertNoCommandRetry(adapter.commands, command: .holdSpeedAtOne)
        assertNoCommandRetry(adapter.commands, command: .restoreSpeed(1.5))
    }

    func testSupersededPendingResumeKeepsPauseOwnershipUntilTrustedConfirmation() async {
        let adapter = FakeIINAPlaybackAdapter(
            snapshots: [
                .playing(speed: 1.5),
                .paused(speed: 1.5),
        ...
        XCTAssertNil(coordinator.state.stoppedReason)
        XCTAssertEqual(coordinator.state.playerSnapshot, .playing(speed: 1.5))
        assertNoCommandRetry(adapter.commands, command: .pause)
        assertNoCommandRetry(adapter.commands, command: .resume)
    }
